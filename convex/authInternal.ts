...
const totpStav = v.object({
  totpSecret: v.optional(v.string()),
  totpAktivni: v.optional(v.boolean()),
  totpZalozniKody: v.optional(v.array(v.string())),
})

/** Stav druhého faktoru u účtu. Volá ji přihlašovací akce. */
export const najdiTotp = internalQuery({
  args: { coachId: v.id("coaches") },
  returns: v.union(totpStav, v.null()),
  handler: async (ctx, args) => {
    const c = await ctx.db.get(args.coachId)
    if (!c) return null
    return {
      totpSecret: c.totpSecret,
      totpAktivni: c.totpAktivni,
      totpZalozniKody: c.totpZalozniKody,
    }
  },
})

/** Zapne druhý faktor. Volá se až po ověření prvního kódu z aplikace. */
export const zapniTotp = internalMutation({
  args: {
    coachId: v.id("coaches"),
    secret: v.string(),
    zalozniKody: v.array(v.string()),
  },
  returns: v.null(),
  handler: async (ctx, args) => {
    await ctx.db.patch(args.coachId, {
      ...
      totpAktivni: true,
      totpZalozniKody: args.zalozniKody,
      totpZapnutoAt: Date.now(),
    })
    return null
  },
})
...
/**
 * Vypne druhý faktor a zahodí tajemství.
 *
 * Používá se jak při vlastním vypnutí, tak při resetu hesla masterem: kdo
 * ztratil telefon i záložní kódy, se jinak k účtu nedostane.
 */
export const vypniTotpInterne = internalMutation({
  args: { coachId: v.id("coaches") },
  returns: v.null(),
  handler: async (ctx, args) => {
    await ctx.db.patch(args.coachId, {
      ...
      totpAktivni: false,
      totpZalozniKody: undefined,
      totpZapnutoAt: undefined,
    })
    return null
  },
})

/** Spotřebuje záložní kód: každý platí jen jednou. */
export const spotrebujZalozniKod = internalMutation({
  args: { coachId: v.id("coaches"), otisk: v.string() },
  returns: v.null(),
  handler: async (ctx, args) => {
    const c = await ctx.db.get(args.coachId)
    if (!c?.totpZalozniKody) return null
    await ctx.db.patch(args.coachId, {
      totpZalozniKody: c.totpZalozniKody.filter((k) => k !== args.otisk),
    })
    return null
  },
})
